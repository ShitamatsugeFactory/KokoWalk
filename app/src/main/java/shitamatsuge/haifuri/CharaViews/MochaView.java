 ...
 */
public class MochaView extends CharaView {
    private String TAG = "CharaView";
    int [] moveLightImageId = {
            R.drawable.mocha_off, R.drawable.mocha_off_r,
            R.drawable.mocha_on, R.drawable.mocha_on_r,
    };

    public MochaView(Context context, AttributeSet attrs) {
        super(context, attrs);
        View view = LayoutInflater.from(context).inflate(R.layout.mocha, this);
    ...
    protected void setRandomActionImage() {
        ...
        switch (r) {
            case 0:
                mRandomAction[0].setBackgroundResource(R.drawable.mocha_on);
                mRandomAction[1].setBackgroundResource(R.drawable.mocha_on_r);
                break;
            default:
                mRandomAction[0].setBackgroundResource(R.drawable.mocha_off);
                mRandomAction[1].setBackgroundResource(R.drawable.mocha_off_r);
                break;
        }
    }
    ...
    public void upperAnimation(int mSec) {
        ...
        if (mLock) {
            ...
            for(int i = 0; mSec > currentSec; i++) {
                final int index = (i % moveLightImageId.length / 2) * 2;
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        mTouchUpper[mDirection].setBackgroundDrawable(getResources().getDrawable(moveLightImageId[index + mDirection]));
